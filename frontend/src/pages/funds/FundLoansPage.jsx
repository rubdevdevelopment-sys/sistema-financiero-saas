...
export function FundLoansPage() {
  ...
  const projected = useMemo(() => {
    ...
    const installment = total / Number(form.installment_count || 1);
  ...
  const totals = (loansQuery.data ?? []).reduce(
    (acc, loan) => ({
      ...
      interest: acc.interest + Number(loan.total_amount - loan.principal_amount || 0),
      pending: acc.pending + Number(loan.outstanding_balance || 0),
      overdue: acc.overdue + (loan.status === "overdue" ? 1 : 0)
    }),
    { activePortfolio: 0, interest: 0, pending: 0, overdue: 0 }
  );
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Crea prestamos simples, apruebalos, genera cuotas y registra pagos sin amortizacion bancaria compleja."
      />

      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <StatCard label="Cartera activa" value={totals.activePortfolio} accent="bg-brand-500" />
        <StatCard label="Intereses generados" value={totals.interest} accent="bg-emerald-500" />
        ...
        <form className="panel-soft p-6" onSubmit={submit}>
          ...
          <div className="mt-5 grid gap-4">
            ...
            <div className="rounded-2xl border border-brand-100 bg-brand-50 px-4 py-3 text-sm text-slate-700">
              Total: <strong>{currency(projected.total)}</strong> | Cuota: <strong>{currency(projected.installment)}</strong>
            </div>
              ...
              <article key={loan.id} className="rounded-2xl border border-slate-100 bg-slate-50 p-4">
                <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                  <div>
                    ...
                    <p className="text-sm text-slate-500">{loan.cycle_name} | {loan.installment_count} cuotas de {currency(loan.installment_value)}</p>
                  </div>
                ...
                <div className="mt-4 grid gap-3 text-sm text-slate-600 md:grid-cols-4">
                  ...
                  <p>Total: {currency(loan.total_amount)}</p>
          ...
          <table className="min-w-full divide-y divide-slate-100 text-sm">
            ...
            <tbody className="divide-y divide-slate-100 bg-white">
              ...
              {!installmentsQuery.isLoading && (installmentsQuery.data ?? []).length === 0 ? (
                <tr><td className="px-4 py-8 text-center text-slate-500" colSpan="7">Selecciona un prestamo para ver sus cuotas.</td></tr>
              ) : null}
            </tbody>
          </table>
        </div>
